...
export default async function EditCategoryPage({ params }: { params: Promise<{ id: string }> }) {
  const t = await getTranslations('AdminEditCategory');
  await verifySession();
    ...
    <div className="space-y-8 max-w-2xl mx-auto">
      ...
      <div className="bg-white/70 backdrop-blur-xl rounded-3xl p-8 shadow-sm border border-white h-fit">
        <div className="flex items-center gap-3 mb-6">
          ...
          <h2 className="font-heading text-2xl text-brand-charcoal">{t('CategoryDetails')}</h2>
        </div>
        ...
        <form action={updateCategoryWithId} className="space-y-5">
          ...
          <div>
            <label htmlFor="image" className="block text-sm font-medium text-brand-charcoal/80 mb-1.5 ml-1">
              {t('FormImage')} {category.imageUrl && t('FormImageReplace')}
            </label>
            {category.imageUrl && (
              <div className="mb-3 w-32 h-32 rounded-xl overflow-hidden border border-brand-pink-100">
                <img src={category.imageUrl} alt={t('AltCurrentCategory')} className="w-full h-full object-cover" />
              </div>
            )}
            <input
              type="file"
