    ...
    <div className="bg-brand-offwhite min-h-screen pt-32 pb-12">
      <div className="max-w-7xl mx-auto px-4 sm:px-6">
        ...
        <nav className="flex flex-wrap items-center gap-2 text-xs font-bold tracking-widest uppercase text-brand-charcoal/50 mb-8 md:mb-12">
          <Link href="/" className="hover:text-brand-pink-500 transition-colors"><span>{t('Home')}</span></Link>
          <ChevronRight className="w-3 h-3" />
          <Link href="/shop" className="hover:text-brand-pink-500 transition-colors"><span>{t('Collection')}</span></Link>
          <ChevronRight className="w-3 h-3" />
          <Link href={`/shop?category=${product.category.slug}`} className="hover:text-brand-pink-500 transition-colors"><span>{product.category.name}</span></Link>
          <ChevronRight className="w-3 h-3" />
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-12 lg:gap-24">
          ...
          <div className="flex flex-col py-4">
            ...
            <div className="prose prose-pink text-brand-charcoal/70 mb-12 font-light leading-relaxed text-lg">
              <p>{product.description || t('DefaultDesc')}</p>
            </div>
            ...
            <div className="space-y-8 mb-12 border-y border-brand-pink-100 py-8">
              {sizes.length > 0 && (
                <div>
                  <div className="flex items-center justify-between mb-4">
                    <h3 className="text-xs font-bold tracking-widest text-brand-charcoal uppercase">{t('SelectSize')}</h3>
              ...
              {colors.length > 0 && (
                <div>
                  <h3 className="text-xs font-bold tracking-widest text-brand-charcoal uppercase mb-4">{t('SelectColor')}</h3>
                  <div className="flex flex-wrap gap-3">
                    {colors.map(color => (
              ...
              {materials.length > 0 && (
                <div>
                  <h3 className="text-xs font-bold tracking-widest text-brand-charcoal uppercase mb-4">{t('Material')}</h3>
                  <div className="flex flex-wrap gap-3">
            ...
            <div className="bg-white p-6 sm:p-8 rounded-3xl border border-brand-pink-100 shadow-xl shadow-brand-pink-50/50 relative overflow-hidden">
              ...
              <h3 className="font-heading text-2xl text-brand-charcoal mb-3 relative z-10">{t('Interested')}</h3>
              ...
              >
                <MessageCircle className="w-5 h-5" /> <span>{t('InquireNow')}</span>
              </Link>
            </div>
            ...
            <div className="mt-12 space-y-4">
              <details className="group bg-white rounded-2xl border border-brand-pink-50 p-6 cursor-pointer">
                <summary className="flex items-center justify-between font-heading text-lg text-brand-charcoal list-none">
                  <span className="flex items-center gap-3"><Info className="w-5 h-5 text-brand-pink-400" /> <span>{t('ShippingReturns')}</span></span>
                ...
                <div className="mt-4 text-brand-charcoal/60 font-light text-sm leading-relaxed pl-8">
                  {t('ShippingReturnsDesc')}
                </div>
              </details>
              
              <details className="group bg-white rounded-2xl border border-brand-pink-50 p-6 cursor-pointer">
                <summary className="flex items-center justify-between font-heading text-lg text-brand-charcoal list-none">
                  <span className="flex items-center gap-3"><ShieldCheck className="w-5 h-5 text-brand-pink-400" /> <span>{t('CareInstructions')}</span></span>
                  <ChevronRight className="w-5 h-5 text-brand-charcoal/40 group-open:rotate-90 transition-transform" />
                </summary>
                <div className="mt-4 text-brand-charcoal/60 font-light text-sm leading-relaxed pl-8">
                  {t('CareInstructionsDesc')}
                </div>
              </details>
            </div>
          </div>
